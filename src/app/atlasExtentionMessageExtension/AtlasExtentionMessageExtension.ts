...
@PreventIframe("/atlasExtentionMessageExtension/config.html")
@PreventIframe("/atlasExtentionMessageExtension/action.html")
export default class AtlasExtentionMessageExtension implements IMessagingExtensionMiddlewareProcessor {
    ...
    public async onQuerySettingsUrl(context: TurnContext): Promise<{ title: string, value: string }> {
        return Promise.resolve({
            title: "Atlas Extention Configuration",
            value: `https://${process.env.HOSTNAME}/atlasExtentionMessageExtension/config.html?name={loginHint}&tenant={tid}&group={groupId}&theme={theme}`
        });
    }
